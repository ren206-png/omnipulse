    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
          ...
          <p style="color:#4b5563">${escapeHtml(opts.inviterEmail)} has invited you to join <strong>${escapeHtml(opts.workspaceName)}</strong> as a <strong>${escapeHtml(opts.role)}</strong>.</p>
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
          ...
          <p style="color:#4b5563"><strong>${escapeHtml(opts.submitterEmail)}</strong> submitted a post for approval in <strong>${escapeHtml(opts.workspaceName)}</strong>.</p>
          <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:16px;margin:16px 0">
            <p style="color:#374151;margin:0;font-size:14px;white-space:pre-wrap">${escapeHtml(opts.postContent.slice(0, 280))}${opts.postContent.length > 280 ? '…' : ''}</p>
          </div>
          ...
          <p style="color:#9ca3af;font-size:12px;margin-top:24px">You received this because you are an admin or owner of ${escapeHtml(opts.workspaceName)}.</p>
        </div>
      `,
    })
  } catch (err) {
      ...
      html: `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
          ...
          <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:16px;margin:16px 0">
            ...
            <p style="color:#374151;margin:0;font-size:14px">${escapeHtml(opts.note)}</p>
          </div>` : ''}
